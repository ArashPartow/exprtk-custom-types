 ...
 *                                                            *
 * ExprTk MPFR vs Native Benchmarks                           *
...
{
   exprtk::parser<numeric_type> parser;

   for (std::size_t i = 0; i < global_expression_list_size; ++i)
   {
      exprtk::expression<numeric_type> expression;
...
struct native
{
   ...
   {
      return mpfr::sin(T(2) * x) + mpfr::cos(pi / y);
   }

   static inline T func10(Type x, Type y)
   {
      return T(1) - mpfr::sin(T(2) * x) + mpfr::cos(pi / y);
   }

   static inline T func11(Type x, Type y)
   {
      return mpfr::sqrt(T(111.111) - mpfr::sin(T(2) * x) + mpfr::cos(pi / y) / T(333.333));
   }

   static inline T func12(Type x, Type y)
   {
      return ((x * x) / mpfr::sin(T(2) * pi / y)) - x / T(2);
   }

   static inline T func13(Type x, Type y)
   {
      return (x + (mpfr::cos(y - mpfr::sin(T(2) / x * pi)) - mpfr::sin(x - mpfr::cos(T(2) * y / pi))) - y);
   }

   static inline T func14(Type x, Type y)
   {
      return clamp(T(-1), mpfr::sin(T(2) * pi * x) + mpfr::cos(y / T(2) * pi), + T(1));
   }

   static inline T func15(Type x, Type y)
   {
      return mpfr::max(T(3.33), mpfr::min(sqrt(T(1) - mpfr::sin(T(2) * x) + mpfr::cos(pi / y) / T(3)), T(1.11)));
   }
...
int main(int argc, char* argv[])
{
   ...
   exprtk::symbol_table<numeric_type> symbol_table;
   symbol_table.add_constants();
   symbol_table.add_variable("x",x);
   symbol_table.add_variable("y",y);

   std::deque<exprtk::expression<numeric_type> > compiled_expr_list;
...

   {
      ...
      run_native_benchmark(x,y,native<numeric_type>::func00,global_expression_list[ 0]);
      run_native_benchmark(x,y,native<numeric_type>::func01,global_expression_list[ 1]);
      run_native_benchmark(x,y,native<numeric_type>::func02,global_expression_list[ 2]);
      run_native_benchmark(x,y,native<numeric_type>::func03,global_expression_list[ 3]);
      run_native_benchmark(x,y,native<numeric_type>::func04,global_expression_list[ 4]);
      run_native_benchmark(x,y,native<numeric_type>::func05,global_expression_list[ 5]);
      run_native_benchmark(x,y,native<numeric_type>::func06,global_expression_list[ 6]);
      run_native_benchmark(x,y,native<numeric_type>::func07,global_expression_list[ 7]);
      run_native_benchmark(x,y,native<numeric_type>::func08,global_expression_list[ 8]);
      run_native_benchmark(x,y,native<numeric_type>::func09,global_expression_list[ 9]);
      run_native_benchmark(x,y,native<numeric_type>::func10,global_expression_list[10]);
      run_native_benchmark(x,y,native<numeric_type>::func11,global_expression_list[11]);
      run_native_benchmark(x,y,native<numeric_type>::func12,global_expression_list[12]);
      run_native_benchmark(x,y,native<numeric_type>::func13,global_expression_list[13]);
      run_native_benchmark(x,y,native<numeric_type>::func14,global_expression_list[14]);
      run_native_benchmark(x,y,native<numeric_type>::func15,global_expression_list[15]);
      run_native_benchmark(x,y,native<numeric_type>::func16,global_expression_list[16]);
   }
...
void pgo_primer()
{
   exprtk::pgo_primer<numeric_type>();
   ...
   numeric_type total = 0.0;

   for (numeric_type x = lower_bound_x; x <= upper_bound_x; x += delta)
   {
      for (numeric_type y = lower_bound_y; y <= upper_bound_y; y += delta)
      {
         total += native<numeric_type>::func00(x,y);
         total += native<numeric_type>::func01(x,y);
         total += native<numeric_type>::func02(x,y);
         total += native<numeric_type>::func03(x,y);
         total += native<numeric_type>::func04(x,y);
         total += native<numeric_type>::func05(x,y);
         total += native<numeric_type>::func06(x,y);
         total += native<numeric_type>::func07(x,y);
         total += native<numeric_type>::func08(x,y);
         total += native<numeric_type>::func09(x,y);
         total += native<numeric_type>::func10(x,y);
         total += native<numeric_type>::func11(x,y);
         total += native<numeric_type>::func12(x,y);
         total += native<numeric_type>::func13(x,y);
         total += native<numeric_type>::func14(x,y);
         total += native<numeric_type>::func15(x,y);
         total += native<numeric_type>::func16(x,y);
      }
   }
}
...
void perform_file_based_benchmark(const std::string& file_name, const std::size_t& rounds)
{
   ...
   numeric_type b = 2.2;
   numeric_type c = 3.3;
   numeric_type x = 2.123456;
   numeric_type y = 3.123456;
   numeric_type z = 4.123456;
   numeric_type w = 5.123456;
   ...
   exprtk::polynomial<numeric_type, 1> poly01;
   exprtk::polynomial<numeric_type, 2> poly02;
   exprtk::polynomial<numeric_type, 3> poly03;
   exprtk::polynomial<numeric_type, 4> poly04;
   exprtk::polynomial<numeric_type, 5> poly05;
   exprtk::polynomial<numeric_type, 6> poly06;
   exprtk::polynomial<numeric_type, 7> poly07;
   exprtk::polynomial<numeric_type, 8> poly08;
   exprtk::polynomial<numeric_type, 9> poly09;
   exprtk::polynomial<numeric_type,10> poly10;
   exprtk::polynomial<numeric_type,11> poly11;
   exprtk::polynomial<numeric_type,12> poly12;
   ...
   static numeric_type e = exprtk::details::numeric::constant::e;
   ...
   for (std::size_t i = 0; i < expression_list.size(); ++i)
   {
      ...
      printf("Expression %3d of %3d %9.3f ns\t%10d ns\t(%.30s)  '%s'\n",
             static_cast<int>(i + 1),
