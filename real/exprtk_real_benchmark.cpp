...
{
   exprtk::parser<real::type> parser;
   for (std::size_t i = 0; i < expression_list_size; ++i)
   {
      exprtk::expression<real::type> expression;
...
void run_native_benchmark(T& x, T& y, NativeFunction f, const std::string& expr_string)
{
   ...
   for (x = lower_bound_x_d; x <= upper_bound_x_d; x += delta_d)
   {
      for (y = lower_bound_y_d; y <= upper_bound_y_d; y += delta_d)
      {
         total += f(x,y);
         ++count;
      }
...
struct native
{
   ...
   {
      return std::sin(T(2) * x) + std::cos(pi_d / y);
   }

   static inline T func10(Type x, Type y)
   {
      return T(1) - std::sin(T(2) * x) + std::cos(pi_d / y);
   }

   static inline T func11(Type x, Type y)
   {
      return std::sqrt(T(111.111) - std::sin(T(2) * x) + std::cos(pi_d / y) / T(333.333));
   }

   static inline T func12(Type x, Type y)
   {
      return ((x * x) / std::sin(T(2) * pi_d / y)) - x / T(2);
   }

   static inline T func13(Type x, Type y)
   {
      return (x + (std::cos(y - std::sin(T(2) / x * pi_d)) - std::sin(x - std::cos(T(2) * y / pi_d))) - y);
   }

   static inline T func14(Type x, Type y)
   {
      return clamp(T(-1), std::sin(T(2) * pi_d * x) + std::cos(y / T(2) * pi_d), + T(1));
   }

   static inline T func15(Type x, Type y)
   {
      return std::max(T(3.33), std::min(sqrt(T(1) - std::sin(T(2) * x) + std::cos(pi_d / y) / T(3)), T(1.11)));
   }

   static inline T func16(Type x, Type y)
   {
      return (((y + (x * T(2.2))) <= (x + y + T(1.1))) ? x - y : x * y) + T(2) * pi_d / x;
   }
};
...
int main(int argc, char* argv[])
{
   ...
   exprtk::symbol_table<real::type> symbol_table;
   symbol_table.add_constants();
   symbol_table.add_variable("x",x);
   symbol_table.add_variable("y",y);

   std::deque<exprtk::expression<real::type> > expr_list;
...
void pgo_primer()
{
   ...
   static const real::type upper_bound_y = +50.0;
   static const real::type delta = 0.07;

   real::type total = 0.0;

   for (real::type x = lower_bound_x; x <= upper_bound_x; x += delta)
   {
      for (real::type y = lower_bound_y; y <= upper_bound_y; y += delta)
      {
...
void perform_file_based_benchmark(const std::string& file_name, const std::size_t& rounds)
{
   ...
   typedef exprtk::symbol_table<real::type> symbol_table_t;
   typedef exprtk::expression<real::type>     expression_t;
   typedef exprtk::parser<real::type>             parser_t;
   ...
   real::type b = 2.2;
   real::type c = 3.3;
   real::type x = 2.123456;
   real::type y = 3.123456;
   real::type z = 4.123456;
   real::type w = 5.123456;
   ...
   exprtk::polynomial<real::type, 1> poly01;
   exprtk::polynomial<real::type, 2> poly02;
   exprtk::polynomial<real::type, 3> poly03;
   exprtk::polynomial<real::type, 4> poly04;
   exprtk::polynomial<real::type, 5> poly05;
   exprtk::polynomial<real::type, 6> poly06;
   exprtk::polynomial<real::type, 7> poly07;
   exprtk::polynomial<real::type, 8> poly08;
   exprtk::polynomial<real::type, 9> poly09;
   exprtk::polynomial<real::type,10> poly10;
   exprtk::polynomial<real::type,11> poly11;
   exprtk::polynomial<real::type,12> poly12;
   ...
   static real::type e = exprtk::details::numeric::constant::e;
   ...
   real::type single_eval_total_time = 0.0;
   ...
   for (std::size_t i = 0; i < expression_list.size(); ++i)
   {
      ...
      b = real::type(2.2);
      c = real::type(3.3);
      x = real::type(2.123456);
      y = real::type(3.123456);
      z = real::type(4.123456);
      w = real::type(5.123456);
